 ...
 *
 * A panel rather than its own dialog since the surfaces were consolidated: a
 * saved script is a thing that RUNS, so it belongs beside the automations
 * rather than in a drawer of its own. What must not blur in the move is that
 * these can act on the machine — hence the auto-run switch stays inside this
 * panel, governing only what is listed here.
 */
export function RecipesPanel({ open }) {
        ...
        {editing ? (
          ...
          <>
            {/* One switch for the machine, next to the per-recipe ones it
                ...
                should say so where it is ticked. It stays in THIS panel and
                names what it covers: nothing on the Automations tab, and
                nothing in Skills, is governed by it. */}
            ...
            <p className="mb-1.5 text-xs font-medium text-muted-foreground">
              Built in — always available, not editable
            </p>
            <ul className="flex flex-col gap-1.5">
              {builtin.map((r) => (
                <li key={r.name} className="rounded-md border border-dashed px-3 py-2">
                  <div className="flex items-center gap-2">
                    <button
                      className="flex min-w-0 flex-1 items-center gap-2 text-left"
                      onClick={() => setOpenName(openName === r.name ? null : r.name)}
      ...
      <div className="flex shrink-0 justify-end gap-2">
        {editing ? (
          <>
            ...
            <Button
              disabled={
                busy || !editing.name.trim() || !editing.summary.trim() || !editing.script.trim()
              }
              onClick={commit}
            >
