...
export const ABSTAIN_PHRASES =
  /\b(i don'?t have|i do not have|nothing (on|in|about)|no (record|mention|reference|information|trace|sign) (of|about|available)|no information|i don'?t know|not something i know|can'?t find|couldn'?t find|could not find|cannot find|unable to (find|locate)|(is|was|were) not found|not found|isn'?t (here|there|in the workspace)|is not (here|there|present|available|defined|in the workspace)|does not (exist|appear)|doesn'?t (exist|appear)|no (such )?(file|function|folder)|no matches|not (in|among) the (files|workspace|project|codebase))\b/i;
